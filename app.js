  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    con.query(query, [email], (err, result) => {
      ...
      if (result[0].role === "Faculty") {
        ...
        return res.send({ message: "Unauthorized login : Not a faculty" });
      }
    });
      ...
      if (result.length === 0) {
        ...
        con.query(insertQuery, [email], (err, result) => {
          if (err) {
            return res
              .status(500)
              .send({ message: "Failed to insert faculty" });
          }
      ...
      if (result.length === 0) {
        ...
        con.query(insertQuery, [email], (err, result) => {
          if (err) {
            return res
              .status(500)
              .send({ message: "Failed to insert student" });
          }
...
app.post("/enrollCoursesFaculty", (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    if (!courseId) {
      return res.status(400).send({ message: "No course selected" });
    }
    ...
    con.query(enrollmentQuery, [email, courseId], (err, result) => {
      if (err) {
        return res.status(500).send({ message: "Failed to assign course" });
      }
...
app.post("/deleteCourse/:course_id", (req, res) => {
  const courseId = req.params.course_id;
  const token = req.cookies.token;
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    con.query(query, [email], (err, result) => {
      ...
      if (result[0].role === "Admin") {
        return res.render("admin", { email: email });
      } else {
        return res.send({ message: "Unauthorized login: Not an admin" });
      }
    });
  });
});
...
//           if (err) return res.status(500).send({ message: 'Database query failed' });
  ...
  con.query(userRoleQuery, [userId], (err, results) => {
    ...
    if (role === "Student") {
      ...
      const deleteStudentCoursesQuery =
        "DELETE FROM student_courses WHERE email = ?";
      con.query(deleteStudentCoursesQuery, [userId], (err) => {
        if (err) {
          return res.status(500).send("Error deleting student courses: " + err);
        }
      ...
      // Delete associated records from instructor_courses table
      deleteAssociatedRecordsQuery =
        "DELETE FROM instructor_courses WHERE instructor_email = ?";
      // Then delete the instructor from the Instructor table
      const deleteInstructorQuery = "DELETE FROM Instructor WHERE email = ?";
      con.query(deleteAssociatedRecordsQuery, [userId], (err) => {
        if (err) {
          return res
            .status(500)
        ...
        con.query(deleteInstructorQuery, [userId], (err) => {
          if (err) {
            return res.status(500).send("Error deleting instructor: " + err);
          }
          // Now delete the user
          const deleteUserQuery = "DELETE FROM Users WHERE email = ?";
          con.query(deleteUserQuery, [userId], (err) => {
